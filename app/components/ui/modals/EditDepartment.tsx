...
type EditableDepartment = Omit<Department, "lead"> & {
  leadName: string;
...
export default function EditDepartmentModal({
  ...
  const [formData, setFormData] = useState<EditableDepartment>({
    ...department,
    leadName: department.lead
      ? `${department.lead.first_name} ${department.lead.last_name}`.trim()
      : "",
    contact_email: department.contact_email || "",
  });

  const handleChange = (field: keyof EditableDepartment, value: string) => {
    setFormData((prev) => ({ ...prev, [field]: value }));
  };

  const handleUpdate = () => {
    // Prepare updated Department object here, mapping leadName back to lead if needed
    const updatedDept: Department = {
      ...formData,
      lead: department.lead, // optionally keep original lead object or update accordingly
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white/60 backdrop-blur-md flex items-center justify-center px-4 overflow-y-auto">
      <div className="relative w-full bg-white rounded-2xl shadow-2xl p-6 md:p-10 max-h-[90vh] overflow-y-auto max-w-lg">
        ...
        <h2 className="text-xl font-semibold text-gray-900 mb-6">Edit Department</h2>
        ...
        <div className="space-y-4">
          ...
          <InputField
            ...
            onChange={(v) => handleChange("leadName", v)}
          />
          <InputField
            label="Email *"
