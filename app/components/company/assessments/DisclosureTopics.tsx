...
import { ArrowLeft, ChevronRight, Info, Search, CheckCircle2 } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function DisclosureTopics({
  ...
  // Calculate completion status for all topics whenever assessment data changes
  useEffect(() => {
    const allTopics = [
      ...industrySpecificMetrics.flatMap((section) => section.cards.map((card) => card.title)),
      ...supplementaryMetrics.flatMap((section) => section.cards.map((card) => card.title)),
    ];

    const statusMap: Record<string, CompletionStatus> = {};
    allTopics.forEach((topicTitle) => {
      statusMap[topicTitle] = checkTopicCompletion(topicTitle, state.assessmentData);
    });

    setTopicCompletionStatus(statusMap);
  ...
  const renderCompletionIndicator = (cardTitle: string) => {
    const status = topicCompletionStatus[cardTitle];
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="space-y-2 flex-1">
                                        <div className="flex items-center justify-between">
                                          ...
                                          {renderCompletionIndicator(card.title)}
                                        </div>
                                        <p className="text-sm text-muted-foreground">
                                          {card.subtitle}
                                        </p>
