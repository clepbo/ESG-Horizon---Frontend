...
//   publicId: string; // Add publicId here
// }
...
import { Input } from "@/app/components/ui/input";
import { Label } from "@/app/components/ui/label";
import { CloudUpload, X, Plus, Trash2, CheckCircle2 } from "lucide-react";
import { toast } from "react-toastify"; // Using react-toastify as in the example
import { LoadingSpinner } from "@/app/components/ui/loading-spinner";
import { uploadService } from "@/services/upload.service";
import { FileData } from "@/hooks/useAssessment";

// The `FileData` interface with updated props
...
export function AdditionalFileUpload({
  ...
  const handleAddField = () => {
    const newFields = [
      ...additionalFields,
      { name: "", url: null, publicId: null, size: 0, lastModified: 0 },
    ];
    setAdditionalFields(newFields);
    onFieldsChange?.(newFields);
  };
  ...
  const handleRemoveField = async (index: number) => {
    const fileToRemove = additionalFields[index];

    if (fileToRemove?.publicId) {
      try {
        await uploadService.deleteImage(fileToRemove.publicId);
  ...
  const handleRemoveFile = async (index: number) => {
    ...
    const newFields = additionalFields.map((item, i) =>
      i === index ? { ...item, url: null, publicId: null, name: "" } : item
    );
    setAdditionalFields(newFields);
    onFieldsChange?.(newFields);
  };
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="flex-1 border-gray-300 text-gray-600 hover:bg-gray-50 flex items-center justify-between"
                onClick={() => {
                  document.getElementById(`additional-file-${index}`)?.click();
                }}
              >
                <div className="flex items-center">
                  {uploading[index] ? (
                    ...
                    <>
                      ...
                      <span className="truncate">{fieldData.name}</span>
                    </>
                  ) : (
                    <>
                      <CloudUpload className="h-4 w-4 mr-2" />
