...
const fuelTypes = ["Diesel", "Petrol", "Natural Gas"]; // Placeholder
const uploadFields = [
...
export function RoadTransport({
  ...
  useEffect(() => {
    const existingData =
      ...
      JSON.parse(localStorage.getItem("mobileSources.roadTransport") || "{}");
    if (existingData) {
  ...
  const handleSaveAndContinue = () => {
    ...
    localStorage.setItem("mobileSources.roadTransport", JSON.stringify(payload));
    setIsSaving(false);
  ...
  const handleNext = () => {
    ...
    localStorage.setItem("mobileSources.roadTransport", JSON.stringify({ selectedFuelType, fuelVolume, files }));
    onNext();
  };
