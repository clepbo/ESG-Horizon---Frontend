...
export function GasFlaring({ onBack, onNext, stepIndex, totalSteps, percent }: GasFlaringProps) {
...
//   useEffect(() => {
//     const existingData = state.assessmentData.processEmissions?.gasFlaring || {};
...
//   }, [state.assessmentData.processEmissions?.gasFlaring]);
  ...
  const handleFileChange = (field: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setFiles((prev) => ({ ...prev, [field]: { name: file.name, size: file.size, lastModified: file.lastModified } }));
          ...
          <CardContent className="space-y-8">
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-gray-500">Section {stepIndex} of {totalSteps}</span>
                <span className="text-sm font-medium text-gray-500">{percent}% complete</span>
              </div>
              <div className="w-full h-3 bg-green-300 rounded-lg">
                <div className="h-3 bg-green-800 rounded transition-all duration-300" style={{ width: `${percent}%` }} />
              </div>
            </div>
            <div>
              ...
              <div className="space-y-4 ml-6">
                <div className="space-y-2">
                  ...
                  />
                  {errors.gasVolume && <p id="gas-volume-error" className="text-sm text-red-500">{errors.gasVolume}</p>}
                </div>
                <div className="space-y-2">
                  ...
                  {errors.carbonContent && (
                    <p id="carbon-content-error" className="text-sm text-red-500">{errors.carbonContent}</p>
                  )}
                </div>
              </div>
            </div>
                    ...
                    <div key={field} className="flex flex-col gap-2">
                      ...
                      <Card className="p-4 flex flex-col items-center justify-center border border-2 hover:border-solid hover:border-primary transition-all h-32">
                        ...
                        >
                          ...
                          <span className="text-xs text-gray-400 text-center">Upload {field} (Max. 10MB)</span>
                        </Label>
                        <Input
                        ...
                        />
                        {files[field] && (
                          <p className="text-sm text-green-600 mt-2 text-center truncate">Uploaded: {files[field]!.name}</p>
                        )}
                      </Card>
                    </div>
                  ))}
