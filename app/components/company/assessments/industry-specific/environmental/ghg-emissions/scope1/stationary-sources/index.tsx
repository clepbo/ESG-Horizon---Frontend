...
export function StationarySourcesForm({ onBack }: StationarySourcesFormProps) {
    ...
    const [currentStep, setCurrentStep] = useState<
        "overview" | "electricity-heat" | "industrial-processes" | "oil-gas"
    >("overview");
    ...
    const getStepStatus = (stepKey: string) => {
        ...
        switch (stepKey) {
            case "electricity-heat":
                return data?.electricityHeat?.dieselVolume ||
                    data?.electricityHeat?.gasVolume
                    ? "completed"
                    : "pending";
            case "industrial-processes":
                return data?.industrialProcesses?.selectedFuelType &&
                    data?.industrialProcesses?.fuelVolume
                    ? "completed"
                    : "pending";
            case "oil-gas":
                return data?.oilGasSubsidiaries?.selectedFuelType &&
                    data?.oilGasSubsidiaries?.fuelVolume
                    ? "completed"
                    : "pending";
            default:
                return "pending";
    ...
    if (currentStep === "electricity-heat") {
        return (
            <ElectricityHeatForm
                ...
                onNext={() => handleStepNavigation("industrial-processes")}
            />
        );
    }
        ...
        <div className="min-h-screen bg-background p-6">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground">
                        Report emissions from fixed combustion sources
                    </p>
                </div>
                ...
                <div className="grid gap-4">
                    ...
                    >
                            ...
                            <CardTitle className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="flex items-center gap-2">
                                        {getStepStatus("electricity-heat") ===
                                        "completed" ? (
                        ...
                        <CardContent>
                            <p className="text-muted-foreground">
                                Input data for diesel powered generators and gas
                                fired turbines
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card
                        className="cursor-pointer hover:bg-accent/50 transition-all duration-200 hover:shadow-md transform hover:-translate-y-1"
                        onClick={() =>
                            handleStepNavigation("industrial-processes")
                        }
                    >
                        <CardHeader>
                            <CardTitle className="flex items-center justify-between">
                        ...
                        <CardContent>
                            <p className="text-muted-foreground">
                                Specify boiler and furnace fuel types and
                                consumption volumes
                            </p>
                        </CardContent>
                    </Card>
