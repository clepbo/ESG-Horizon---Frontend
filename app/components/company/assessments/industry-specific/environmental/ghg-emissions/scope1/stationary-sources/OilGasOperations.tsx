...
export function OilGasOperations({
    ...
    // Combine pending states to disable buttons during either operation
    ...
    // Corrected 'Save & Continue Later' logic using useSaveAssessment
    const handleSaveAndContinue = () => {
        if (!validateForm()) return;

        // 1. Dispatch the current form's data to the global state
        const payload = {
            onShoreProduction,
            additionalFields,
            files,
        ...
        // 2. Trigger the save mutation
        saveAssessment(state.assessmentData, {
            onSuccess: () => {
            ...
            // The hook already handles the error toast
        });
    };

    // 3. CORRECTED 'Submit' logic using useSubmitAssessment
    const handleSubmit = () => {
        if (!validateForm()) return;

        // 1. Dispatch the current form's data to the global state (final update)
        const payload = {
            onShoreProduction,
            additionalFields,
            files,
        ...
        // 2. Trigger the submit mutation
        submitAssessment(state.assessmentData, {
            onSuccess: () => {
                // The hook already shows "Assessment submitted successfully!"
                // Now, navigate to the success screen/next step
                onSubmit();
            },
            // The hook already handles the error toast
        });
...
//         console.info("Dispatching payload final stationary source step:", payload);

//         // 2. Trigger the save mutation with the now-updated global state
//         saveAssessment(state.assessmentData, {
//             onSuccess: () => {
//                 setShowSaveSuccess(true);
...
//         console.info('submitting payload final stationary source step:', state.assessmentData);

//         // 2. Trigger the save mutation (using useSaveAssessment)
//         saveAssessment(state.assessmentData, {
//             onSuccess: () => {
//                 // onSubmit will navigate to the SuccessScreen
//                 toast.success("Stationary Sources submitted successfully!");
//                 onSubmit();
//             },
//             onError: (error) => {
