...
export function ResidualForm({
  ...
  useEffect(() => {
    ...
    if (existingData) {
      ...
      const ef = existingData.residualMixFactor;
  ...
  const saveForm = async (options: { showToast?: boolean; redirect?: boolean } = {}) => {
    ...
    const resolvedFactor =
      customEmissionFactor ?? getScopeEmissionFactor("residual")?.factor ?? null;

    const payload = {
      electricityConsumed: electricityConsumed.rawValue,
      residualMixFactor: resolvedFactor,
      files,
      ...
      <div className="max-w-4xl mx-auto space-y-6 mt-4">
        <div className="flex items-center gap-6 mb-4">
          <Button
            variant="outline"
            onClick={onBack}
            className="cursor-pointer flex items-center gap-2 bg-white border-teal-600 text-teal-700 hover:bg-green-50"
          >
          ...
          <div>
            ...
            <p className="text-muted-foreground text-base">
              Report electricity purchased using the residual mix emission factor, including
