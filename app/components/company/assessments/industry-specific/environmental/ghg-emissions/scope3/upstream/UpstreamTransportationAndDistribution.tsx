...
interface TransportationErrors {
...
const uploadFields = [
  "Logistics invoices/receipts",
  "Shipping waybills or bills of lading",
  "Fleet management or GPS tracking reports",
];

export function UpstreamTransportationAndDistribution({
  onBack,
  onNext,
  stepIndex,
  totalSteps,
  ...
  const [errors, setErrors] = useState<TransportationErrors>({});
  const [massTransported, setMassTransported] = useState("");
  const [distanceTravelled, setDistanceTravelled] = useState("");
  const [logisticsSpend, setLogisticsSpend] = useState("");

  const [fieldErrors, setFieldErrors] = useState({
    massTransported: false,
  ...
  const handleMassTransportedChange = (value: string) => {
    setMassTransported(value);
    if (fieldErrors.massTransported) {
      setErrors((prev) => ({ ...prev, massTransported: undefined }));
      setFieldErrors((prev) => ({ ...prev, massTransported: false }));
    }
  };

  const handleDistanceTravelledChange = (value: string) => {
    setDistanceTravelled(value);
    if (fieldErrors.distanceTravelled) {
      setErrors((prev) => ({ ...prev, distanceTravelled: undefined }));
      setFieldErrors((prev) => ({ ...prev, distanceTravelled: false }));
    }
  };

  const handleLogisticsSpendChange = (value: string) => {
    setLogisticsSpend(value);
    if (fieldErrors.logisticsSpend) {
        ...
        <Card className="animate-in slide-in-from-bottom-4 duration-500 bg-gray-50 mt-6 mb-8 pt-6">
          <CardContent className="space-y-8">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-base">
                Emissions from transportation and distribution of goods and services in the upstream
                value chain.
              </p>
            </div>
