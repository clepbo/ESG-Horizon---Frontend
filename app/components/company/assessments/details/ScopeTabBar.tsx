...
export function ScopeTabBar({ activeScope, onChange, scope1Total, scope2Total, scope3Total }: ScopeTabBarProps) {
  const tabs: ScopeTab[] = [
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-between px-5 py-3 text-left transition-colors cursor-pointer ${
                isActive
                  ? "bg-gray-50 border-b-2 border-teal-600"
                  : "hover:bg-gray-50/50"
              }`}
            >
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-medium tracking-wide uppercase ${isActive ? "text-gray-700" : "text-gray-400"}`}>
                  {tab.sublabel}
                </span>
              </div>
