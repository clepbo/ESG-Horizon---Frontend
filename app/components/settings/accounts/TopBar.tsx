...
export default function TabBar({ tabs, activeTab, setActiveTab }: TabBarProps) {
  return (
    <div className="flex gap-1 lg:gap-3 border border-gray-200 rounded-lg p-2 bg-white mx-6">
      {tabs.map((tab) => (
        <button
          key={tab}
          onClick={() => setActiveTab(tab)}
          className={`flex-1 px-1 lg:px-6 py-2 rounded border transition-colors cursor-pointer ${
            activeTab === tab
