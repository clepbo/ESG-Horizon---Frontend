...
export default function SubsidiariesPage() {
    ...
    useEffect(() => {
        const fetchSubsidiaries = async () => {
            try {
                setLoading(true);
                const res = await subsidiariesService.getAll();
    ...
    const filteredSubsidiaries = useMemo(() => {
        return subsidiaries.filter((sub) => {
            ...
            const matchesSector =
                sectorFilter === "All" || sub.sector === sectorFilter;

            const matchesIndustry =
                industryFilter === "All" || sub.industry === industryFilter;

            return matchesSearch && matchesSector && matchesIndustry;
        });
    }, [subsidiaries, debouncedSearch, sectorFilter, industryFilter]);
