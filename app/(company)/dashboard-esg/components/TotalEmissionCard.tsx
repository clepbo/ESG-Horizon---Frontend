  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="rounded-2xl bg-gradient-to-br from-[#0d3b3b] to-[#1a5c5c] p-6 text-white flex flex-col justify-between h-full min-h-[180px] cursor-default">
          <p className="text-sm font-semibold tracking-widest uppercase text-emerald-300 break-words">
            Total Emission
          </p>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <ScopeChip
              label="Scope 1"
              value={formatNumberShort(scope1)}
              tooltip={formatNumberFull(scope1)}
            />
            <ScopeChip
              label="Scope 2"
              value={formatNumberShort(scope2)}
              tooltip={formatNumberFull(scope2)}
            />
            <ScopeChip
              label="Scope 3"
              value={formatNumberShort(scope3)}
              tooltip={formatNumberFull(scope3)}
            />
          </div>
        </div>
      </TooltipTrigger>
      <TooltipContent side="bottom" sideOffset={8} className="max-w-xs">
        Total GHG emissions (Scope 1 + 2 + 3) from the latest assessment report.
      </TooltipContent>
    </Tooltip>
  );
}
