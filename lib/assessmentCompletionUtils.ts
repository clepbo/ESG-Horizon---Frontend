...
//     "environment",
//     "biodiversityImpact",
//     "environmentalManagement",  // Added this level
//     "hydrocarbonSpills"
//   ],
...
export function checkSubComponentCompletion(
  ...
  if (!dataPath) {
    console.warn(`⚠️ No path mapping found for component: "${componentTitle}"`);
    return { status: "not-started", completionPercentage: 0 };
  }
...
/**
 * Debug helper to verify paths match actual data structure
 * Use this in development to troubleshoot completion status issues
 */
export function debugCompletionPaths(assessmentData: any, topicTitle?: string) {
  if (typeof window === "undefined") return; // Skip in SSR

  console.group("🔍 Assessment Completion Debug");

  if (topicTitle) {
    const config = TOPIC_CONFIGURATIONS[topicTitle];
    if (!config) {
      console.warn(`Topic "${topicTitle}" not found in configuration`);
      console.groupEnd();
      return;
    }
    ...
    console.log(`Paths:`, config.path);

    // Test each path
    config.path.forEach((path, index) => {
      const data = getNestedValue(assessmentData, path);
      console.log(`  Path ${index} [${path.join(".")}]:`, data ? "✅ Found" : "❌ Not found");
      if (data) {
    ...
    if (config.type === "multi-component" && config.subComponents) {
      console.group("Sub-components:");
      const topicData = getDataFromPaths(assessmentData, config.path);

      config.subComponents.forEach((subPath) => {
        const subData = getNestedValueDotNotation(topicData, subPath);
        console.log(`  ${subPath}:`, subData ? "✅ Found" : "❌ Not found");
      });
      console.groupEnd();
    }

    // Check mapped components
    const mappedComponents = Object.keys(SUB_COMPONENT_MAPPING).filter((title) => {
      const path = SUB_COMPONENT_MAPPING[title];
      return config.path.some((topicPath) =>
        topicPath.every((segment, index) => path[index] === segment)
      );
    });

    if (mappedComponents.length > 0) {
      console.group("Mapped Components:");
      mappedComponents.forEach((title) => {
        const path = SUB_COMPONENT_MAPPING[title];
        const data = getNestedValue(assessmentData, path);
        const status = checkSubComponentCompletion(title, assessmentData);
        console.log(
          `  "${title}":`,
          data ? "✅ Found" : "❌ Not found",
          `| Status: ${status.status} (${status.completionPercentage}%)`
        );
      });
      console.groupEnd();
    }
    ...
    // Debug all topics
    Object.keys(TOPIC_CONFIGURATIONS).forEach((topic) => {
      const status = checkTopicCompletion(topic, assessmentData);
      console.log(`${topic}:`, `${status.status} (${status.completionPercentage}%)`);
    });
  }
